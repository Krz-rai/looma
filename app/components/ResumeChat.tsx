...
export function ResumeChat({ resumeId, className, onCitationClick }: ResumeChatProps) {
  // Store the resume context to extract IDs
  const [resumeContext, setResumeContext] = useState<{
    projects: Record<string, string>;
    bullets: Record<string, string>;
    branches: Record<string, string>;
  }>({ projects: {}, bullets: {}, branches: {} });
  ...
  // Build ID mappings when data changes
  useEffect(() => {
    const buildMappings = async () => {
      ...
      projects.forEach(project => {
        projectMap[project.title] = project._id;
      });
      
      // For bullets and branches, we'll need to extract from the chat response
      // since we don't want to query all of them upfront
  ...
  const formatMessage = (content: string) => {
    ...
    formatted = formatted.replace(
      /\[Project:\s*"([^"]+)"\]\s*\{([^}]+)\}/g, 
      (match, text, id) => {
        console.log('Project citation with ID:', text, 'ID:', id);
        const displayText = text.substring(0, 50).trim();
        return `<span class="inline-flex items-center gap-0.5 px-2 py-1 rounded-md bg-foreground/5 hover:bg-foreground/10 border border-border/50 transition-all text-xs font-medium cursor-pointer" data-type="project" data-text="${text}" data-id="${id}">
          ${displayText}
        </span>`;
      }
    );
    
    // Also handle old format without IDs (fallback)
    formatted = formatted.replace(
      /\[Project: "([^"]+)"\](?!\{)/g, 
      (match, text) => {
        const id = resumeContext.projects[text] || '';
        console.log('Project citation (old format):', text, 'ID:', id);
        return `<span class="inline-flex items-center gap-0.5 px-2 py-1 rounded-md bg-foreground/5 hover:bg-foreground/10 border border-border/50 transition-all text-xs font-medium cursor-pointer" data-type="project" data-text="${text}" data-id="${id}">
          ${text}
        </span>`;
      }
    ...
    formatted = formatted.replace(
      /\[Bullet:\s*"([^"]+?)(?:\.\.\.)?"\]\s*\{([^}]+)\}/g,
      (match, text, id) => {
        console.log('Bullet citation with ID:', text, 'ID:', id);
        ...
        return `<span class="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-muted/60 hover:bg-muted transition-all text-xs cursor-pointer" data-type="bullet" data-text="${text}" data-id="${id}">
    ...
    formatted = formatted.replace(
      /\[Branch:\s*"([^"]+?)(?:\.\.\.)?"\]\s*\{([^}]+)\}/g,
      (match, text, id) => {
        console.log('Branch citation with ID:', text, 'ID:', id);
        let displayText = text.replace(/\.+$/, '').trim();
        displayText = displayText.substring(0, 40) + (displayText.length > 40 ? '...' : '');
        return `<span class="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-foreground/[0.03] hover:bg-foreground/[0.06] transition-all text-xs text-muted-foreground cursor-pointer" data-type="branch" data-text="${text}" data-id="${id}">
          <span class="opacity-50">↪</span> ${displayText}
        </span>`;
      }
    );
    ...
    formatted = formatted.replace(
      /\[(Project|Bullet|Branch):\s*"([^"]+)"[^\{]*\]\s*\{([a-z0-9]+)\}/gi,
      (match, type, text, id) => {
        const lowerType = type.toLowerCase();
        const cleanText = text.replace(/\.+$/, '').trim();
        const displayText = cleanText.substring(0, 40) + (cleanText.length > 40 ? '...' : '');
        
        console.log(`Fallback citation found: ${lowerType}:`, text, 'ID:', id);
        
        if (lowerType === 'project') {
          return `<span class="inline-flex items-center gap-0.5 px-2 py-1 rounded-md bg-foreground/5 hover:bg-foreground/10 border border-border/50 transition-all text-xs font-medium cursor-pointer group/citation relative" data-type="project" data-text="${cleanText}" data-id="${id}">
            ${displayText}
          </span>`;
        } else if (lowerType === 'bullet') {
          return `<span class="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-muted/60 hover:bg-muted transition-all text-xs cursor-pointer group/citation relative" data-type="bullet" data-text="${cleanText}" data-id="${id}">
            <span class="opacity-50">•</span> ${displayText}
          </span>`;
        } else if (lowerType === 'branch') {
          return `<span class="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-foreground/[0.03] hover:bg-foreground/[0.06] transition-all text-xs text-muted-foreground cursor-pointer group/citation relative" data-type="branch" data-text="${cleanText}" data-id="${id}">
            <span class="opacity-50">↪</span> ${displayText}
          </span>`;
        }
        return match;
